...
export function TutorChatWidget({ onAsk, learnerName, raised = false }: Props) {
  ...
  const send = async () => {
    ...
    setPending(true)
    try {
      append({ role: 'tutor', text: await onAsk(text) })
    } catch (error) {
      const text = error instanceof TutorLimitError ? error.message : UNAVAILABLE_MESSAGE
      append({ role: 'tutor', text, failed: true })
    } finally {
      setPending(false)
    }
  }
    ...
    <>
      {open && (
        <section
          ...
          className={`fixed ${raised ? 'bottom-40' : 'bottom-24'} right-4 z-50 flex h-[min(540px,calc(100dvh-11rem))] w-[min(380px,calc(100vw-2rem))] flex-col overflow-hidden rounded-2xl border bg-background shadow-2xl sm:right-6`}
        >
      ...
      <button
        ...
        className={`fixed ${raised ? 'bottom-20' : 'bottom-6'} right-4 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-lg transition-transform hover:scale-105 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-ring/50 sm:right-6`}
      >
