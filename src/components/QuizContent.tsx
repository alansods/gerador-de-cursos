...
import { QuizData } from '@/types/course'

interface QuizContentProps {
  quizData: QuizData
  isEditing?: boolean
  onResult?: (result: { acertos: number; total: number }) => void
}

export function QuizContent({ quizData, isEditing = false, onResult }: QuizContentProps) {
  ...
  useEffect(() => {
    if (!isEditing && !showResults) {
      checkAndShowResults()
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [showFeedbacks, currentQuestionIndex, isEditing, showResults])
  ...
  const handleOptionSelect = (optionId: string) => {
    if (showFeedback || isEditing) return
    setSelectedOptions({ ...selectedOptions, [questionId]: optionId })
  }

  const handleConfirm = () => {
    if (!selectedOption || showFeedback || isEditing) return
  ...
  const checkAndShowResults = () => {
    ...
    if (answered === total && currentQuestionIndex === total - 1) {
      ...
      onResult?.({ acertos: correctAnswers, total })
    }
  }
  ...
  const toggleHint = (qId: string) => {
    setShowHints({ ...showHints, [qId]: !showHints[qId] })
  }
  ...
  return (
    <div className="w-full space-y-4">
      {quizData.questions.map((question, questionIndex) => {
        ...
        if (!isEditing && questionIndex !== currentQuestionIndex) return null
            ...
            <div className="p-5 space-y-5">
              ...
              <div className="space-y-2">
                {question.opcoes.map((option, index) => {
                  ...
                  const dimmed = qShowFeedback && !isSelected && !option.isCorrect

                  return (
                    <div key={option.id}>
                      <button
                        onClick={() => handleOptionSelect(option.id)}
                        disabled={qShowFeedback || isEditing}
                      ...
                      {isSelected && qShowFeedback && option.feedback && (
                        <div
