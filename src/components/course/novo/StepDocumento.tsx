...
import { formatarMegabytes } from '@/lib/validacao-curso'
...
export function StepDocumento({
  ...
  const invalido = mostrarErro && !arquivo
        ...
        <div className="flex flex-col gap-3">
          ...
          {(erro || invalido) && (
            <p role="alert" className="flex items-center gap-1.5 text-xs text-destructive">
              ...
              {erro || 'Envie um documento .docx ou .doc de até 10 MB'}
            </p>
          )}
          ...
          <div className="flex items-center gap-4 rounded-xl border border-primary/30 bg-primary/5 p-4">
            ...
            <span className="min-w-0 flex-1">
              ...
              <span className="block text-xs text-muted-foreground">
                {formatarMegabytes(arquivo.size)}
                {extraindo ? ' · lendo o documento…' : ' · pronto para gerar'}
              </span>
            </span>
            <Button
              type="button"
