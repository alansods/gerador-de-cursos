...
export function ClassicNavbar({
  ...
  const fullProgress = progress.percentage >= 100
  const progressTextColor = fullProgress
    ? 'text-green-600 dark:text-green-400'
    : 'text-blue-600 dark:text-blue-400'
  const progressBarColor = fullProgress ? 'bg-green-600' : 'bg-blue-600'

  const progressBar = (
    <div className="flex flex-col gap-1">
      <div className="flex items-center justify-between text-xs">
        <span className="text-gray-500 dark:text-gray-400">Progresso</span>
        <span className={`font-semibold ${progressTextColor}`}>{progress.percentage}%</span>
      </div>
              ...
              >
                <Home
                  className={`w-4 h-4 shrink-0 ${
                    !currentUnitId ? 'text-secondary-foreground' : 'text-muted-foreground/70'
                  }`}
                />
              ...
              {(course.unidades || []).map((u, index) => {
                ...
                return (
                  <a
                    ...
                    className={`flex items-center gap-2.5 px-3 py-2.5 rounded-md text-[13.5px] leading-tight transition-all duration-150 ${itemStyle}`}
                  >
                    {completed && !isActive ? (
                      <Check
