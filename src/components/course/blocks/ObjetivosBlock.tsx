  ...
  return (
    <div className="mb-4 w-full">
      {item.itensObjetivos && item.itensObjetivos.length > 0 ? (
        <div className="flex items-start gap-6 rounded-2xl border border-(--block-accent,#2563eb)/25 bg-(--block-accent,#2563eb)/8 dark:bg-(--block-accent,#2563eb)/15 p-8">
          <div className="shrink-0 flex items-center justify-center w-16 h-16 rounded-full border-2 border-(--block-accent,#2563eb)">
            <Target className="w-7 h-7 text-(--block-accent,#2563eb)" />
          </div>
