...
interface Conteudo {
  id: string
  tipo: 'paragrafo' | 'subtitulo' | 'titulo' | 'imagem' | 'video'
  conteudo: string
...
export const PreviewCurso: React.FC<PreviewCursoProps> = ({ curso, onVoltar, onGerarSCORM }) => {
  const renderConteudo = (item: Conteudo) => {
    ...
    switch (item.tipo) {
      case 'titulo':
        return (
          <h3 className="text-2xl font-bold text-gray-900 mb-4 mt-8 first:mt-0">{item.conteudo}</h3>
        )

      case 'subtitulo':
        return <h4 className="text-xl font-semibold text-gray-800 mb-3 mt-6">{item.conteudo}</h4>

      case 'imagem':
        return (
          <div className="mb-6">
            {item.fonte && (
              <p className="text-xs text-gray-500 mb-2 font-medium">Fonte: {item.fonte}</p>
            )}
            ...
            {item.legenda && (
              <p className="text-sm text-gray-600 italic text-center mt-2">{item.legenda}</p>
            )}
          </div>
        )
      ...
      case 'video':
        return (
          <div className="mb-6">
            {item.videoTitulo && (
              <h4 className="text-lg font-semibold text-gray-900 mb-3">{item.videoTitulo}</h4>
            )}
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
          ...
          <CardHeader className="pb-4">
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <p className="text-blue-100 text-lg leading-relaxed">{curso.descricao}</p>
              </div>
            </div>
          </CardHeader>
        ...
        <div className="space-y-8">
          {(curso.unidades || []).map((unidade) => (
            <Card key={unidade.id} className="shadow-lg border-0 bg-white/80 backdrop-blur-sm">
          ...
          {(!curso.unidades || curso.unidades.length === 0) && (
            <Card className="shadow-lg border-0 bg-white/80 backdrop-blur-sm">
              <CardContent className="p-12 text-center">
                ...
                <h3 className="text-xl font-semibold text-gray-600 mb-2">Nenhuma unidade criada</h3>
                <p className="text-gray-500">
                  Adicione unidades e conteúdo para ver o preview completo do curso
                </p>
              </CardContent>
            </Card>
          )}
