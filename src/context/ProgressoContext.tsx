...
export const ProgressoProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  ...
  const getAulaStatus = (unidadeId: number, aulaId: number): string => {
    return (
      progresso[`unidade-${unidadeId}`]?.[`aula-${aulaId}`]?.status ||
      "nao-iniciada"
    );
  };
  ...
  const getAulaProgresso = (unidadeId: number, aulaId: number): number => {
    return (
      progresso[`unidade-${unidadeId}`]?.[`aula-${aulaId}`]?.progresso || 0
    );
  };
