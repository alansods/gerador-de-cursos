...
export async function generateCourseFromText(
  ...
  let result: { course: Course; tokenUsage: TokenUsage }

  if (geminiApiKey) {
    result = await generateWithGemini(text, geminiApiKey, mode, layout)
  } else if (openaiApiKey) {
    result = await generateWithOpenAI(text, openaiApiKey, mode, layout)
  } else {
  ...
  return {
    course: applyLayoutToGeneratedCourse(normalized, layout),
...
### 25. technical-sheet — ficha técnica: materiais com quantidade, depois os passos
{
  "title": "string",
