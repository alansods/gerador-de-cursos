...
import { ImagemInterativaBlock } from '@/components/course/blocks/ImagemInterativaBlock'
import type { ConteudoUnidade } from '@/types/gerador-curso'

const item: ConteudoUnidade = {
  id: 'b1',
  ordem: 0,
...
describe('imagem interativa', () => {
  it('abre o popup no ponto clicado e não mostra nada antes', async () => {
    ...
    render(<ImagemInterativaBlock item={item} />)

    expect(screen.queryByRole('dialog')).not.toBeInTheDocument()

    await usuario.click(ponto(1))
  ...
  it('fecha ao clicar fora do popup', async () => {
    ...
    render(
      <div>
        <ImagemInterativaBlock item={item} />
  ...
  it('não fecha ao clicar dentro do próprio popup', async () => {
    ...
    await usuario.click(ponto(1))
    await usuario.click(screen.getByText('Camada externa rígida.'))
