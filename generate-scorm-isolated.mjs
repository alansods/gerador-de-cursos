  ...
  try {
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `Não foi possível carregar JSZip. Verifique se está instalado: npm install jszip`
        )
      }
    }
  }
...
async function cleanupAllTemp() {
  ...
  if (tempPaths.workDir && (await pathExists(tempPaths.workDir))) {
    ...
    cleanupTasks.push(
      fs
        .rm(tempPaths.workDir, { recursive: true, force: true })
  ...
  if (tempPaths.tempCursoFile && (await pathExists(tempPaths.tempCursoFile))) {
    ...
    cleanupTasks.push(
      fs
        .unlink(tempPaths.tempCursoFile)
        .then(() => console.log(`   ✅ ${tempPaths.tempCursoFile} removido`))
        .catch((err) =>
          console.warn(`   ⚠️  Erro ao remover ${tempPaths.tempCursoFile}:`, err.message)
        )
    )
  }
  ...
  if (tempPaths.projectOutDir && (await pathExists(tempPaths.projectOutDir))) {
    ...
    cleanupTasks.push(
      fs
        .rm(tempPaths.projectOutDir, { recursive: true, force: true })
        .then(() => console.log(`   ✅ ${tempPaths.projectOutDir} removido`))
        .catch((err) =>
          console.warn(`   ⚠️  Erro ao remover ${tempPaths.projectOutDir}:`, err.message)
        )
    )
  }
...
async function main() {
  ...
  try {
    ...
    if (!(await pathExists(workOutDir))) {
      throw new Error(
        'Build não foi criado. Diretório out/ não encontrado no diretório de trabalho.'
      )
    }
...
`
  ...
  if (await pathExists(publicSrc)) {
    try {
      ...
      throw new Error(
        `Falha ao copiar public/ de ${publicSrc} para ${publicDest}: ${cpError.message}`
      )
    }
  } else {
  ...
  if (await pathExists(nodeModulesSrc)) {
    ...
    if (isVercel) {
      ...
      throw new Error(
        `node_modules não encontrado em: ${nodeModulesSrc}. Isso é necessário para o build funcionar.`
      )
    } else {
...
async function executeIsolatedBuild(cursoFile, workDir) {
  ...
  if (isVercel) {
    ...
    try {
      ...
      console.error(
        '   Isso geralmente significa que o Next.js foi removido pelo tree shaking da Vercel'
      )
      console.error(
        '   Verifique se next.config.ts inclui "./node_modules/next/**/*" em outputFileTracingIncludes'
      )
  ...
  return new Promise((resolve, reject) => {
    ...
    const env = {
      ...
      SCORM_BUILD_COURSE_FILE: workCursoFile, // ✅ Caminho no diretório isolado
    ...
    buildProcess.on('close', async (code) => {
      try {
        ...
        if (code === 0) {
          ...
          if (await pathExists(scormNextDir)) {
            const hasHtmlFiles = await fs
              .readdir(scormNextDir)
              .then((files) => files.some((f) => f.endsWith('.html')))
              .catch(() => false)
...
async function copyBuildFilesToZip(zip, outDir, publicImagesDir, curso) {
  ...
  function convertPaths(html, prefix) {
    ...
    html = html.replace(
      /(href|src|content|data-src|data-href)="\/(?!\/|http|https|mailto|tel|#)/g,
      `$1="${normalizedPrefix}`
    )
    html = html.replace(
      /(href|src|content|data-src|data-href)='\/(?!\/|http|https|mailto|tel|#)/g,
          ...
          if (!shouldExclude) {
            ...
            if (fileName.startsWith('webpack-') && fileName.endsWith('.js')) {
              ...
              if (publicPathRegex.test(jsContent)) {
                ...
                console.warn(
                  '      ⚠️  Não foi possível encontrar a definição do publicPath no Webpack Runtime'
                )
